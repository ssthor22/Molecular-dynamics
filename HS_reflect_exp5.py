...
# Simulate hard sphere motion in box with reflective boundaries
# Hard sphere initialized in FCC configuration

# LATEST WORKING FCC HS
# Scan throught phi = [0.1,0.3,0.49,0.5,0.52,0.6,0.74], 
#		L =   [16.5*r, 11.4*r, 9.7*r, 9.65*r, 9.5*r] 

# species tags --- 1: Ne, 2:Ar
...
def PutInBox(Ri,Vi,L):
	
	for k in range(3):
		if Ri[k] >= (L/2) - r or Ri[k] <= -(L/2) + r:
			Vi[k] = Vi[k]*-1
...
def InitPositionCubic(N, L, split = 0.0):
	...
	a = (2**0.5)*r;
	added = 0;
	for x in range(Ncube):
		for y in range(Ncube):
			for z in range(Ncube):
				position[added, 0] = 2*a*x;
				position[added, 1] = a*y;
				position[added, 2] = a*z;
				added += 1;
	shifted = 0;
	for i in range(0, len(position), 2):
		position[i, 0] += a;
	for i in range(0, len(position), 2*Ncube):
		position[i:i+Ncube, 0] += a;
	"""
	added = 0;
	for x in range(3):
		for y in range(3):
			for z in range(3):
				position[added] = np.array([x, y, z])
				position[added+1] = np.array([x+0.5, y+0.5, z])
				position[added+2] = np.array([x+0.5, y, z+0.5])
				position[added+3] = np.array([x, y+0.5, z+0.5])
				added += 4;
	position *= 2*r;
	position -= L/4;
#	print(position)
...
#		print i_collide_times
		...
		if t < sim_tij: # no collision occurs, just update positions
			...
			for particle in range(N):
				PutInBox( nR[particle],V[particle],L )
			nV = V.copy()
...
def compute_PCF(R_alltimes,steps,L,L_i):
	...
	pickle.dump( PCF_avg, open('PCF_avg_'+str(L_i)+'.p','wb') )
	pickle.dump( PCF_dist, open('PCF_dist_'+str(L_i)+'.p','wb') )
	...
	plt.ylabel('g(r)')
	plt.xlim([0,L/2])
	plt.savefig('PCF_'+str(L_i)+'.png',dpi=300)
	plt.clf()
...
#	raw_input()
	...
	f = open('log.log','wb')
	steps = 20000
	L_values = [16.5*r, 11.4*r, 9.7*r, 9.65*r, 9.5*r]
...
#		PAIR CORRELATION STUFF
		compute_PCF(R_alltimes,steps,L,i)
